...
pub async fn event_handler(
    ...
    framework: &mut poise::FrameworkContext<'_, Data, Error>,
    data: &mut Data,
) -> Result<(), Error>{
    match event {
        ...
        VoiceStateUpdate { new, old} => {
            if new.member.is_some() &&
                ...
                data.meeting_time = false
                
            } else if data.meeting_time && new.channel_id.is_some() && new.member.is_some(){
